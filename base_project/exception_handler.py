...
class ExceptionHandler(Exception):
    ...
    def handle(cls, ex, req, resp, params):  # pylint: disable=W0613
        ...
        if isinstance(ex, InvalidJSON):
            logger.warning("Error with the json schema during handling the %s.", log_msg)  # noqa
            ex = falcon.HTTPUnprocessableEntity(description=str(ex))
        elif isinstance(ex, UnauthorizedException):
            logger.warning("Unauthorized Error of request on %s.", log_msg)
            ...
            logger.warning("Cannot update/delete this record of Category: %s.", log_msg)  # noqa
            ex = falcon.HTTPBadRequest(description=str(ex))
        else:
            logger.warning("Unexpected Error during handling the %s.", log_msg)  # noqa
